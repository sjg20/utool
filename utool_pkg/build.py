...
from utool_pkg.util import exec_cmd, setup_uboot_dir
...
def run_objdump(build_dir, board, args):
    ...
    count = 0
    for elf_path in get_elf_targets(build_dir):
...
def show_size(build_dir, args):
    ...
    Args:
        ...
        args (argparse.Namespace): Arguments from cmdline
    """
    elf_files = list(get_elf_targets(build_dir))
    if not elf_files:
    ...
    result = command.run_one('size', *elf_files)
    print(result.stdout)
...
def build_cmd(args, board, build_dir):
...
def do_build(args):
    ...
    build_dir = args.output_dir or get_build_dir(board)
    ...
    cmd = build_cmd(args, board, build_dir)
